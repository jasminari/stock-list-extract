...
## 2026-08-30 업데이트 — 상승이유 매칭 개선 + 저녁 2차 패스 배포 완료

- [x] `scripts/enrich-reasons.mjs` 갱신 전송 (2순위 「<종목명> 주가」 검색 + 시세 자동기사 배제). 백업: `enrich-reasons.mjs.bak.20260830003854`
- [x] VM에서 `systemd-run`으로 실환경 `--dry` 검증 (8/26 기준 신규 2건, 오류 0)
- [x] **신규 유닛** `stock-enrich.{service,timer}` — 평일 20:00 KST, 1차 패스에서 비었던 종목만 재시도
- [x] 타이머 등록 확인: `stock-extract` 15:40 / `stock-enrich` 20:00 (둘 다 Asia/Seoul)
- [x] 최근 2주 백필 완료 → **등락률 20%+ 급등주 채움률 39.1% → 63.0%**

**왜 2차 패스인가**: 특징주 기사는 장 마감 뒤에도 계속 나온다. 8/28 미채움 종목을 추적해보니
금호전기 기사는 17:14, 한국콜마는 18:18 발행이었다. 15:40 1차 패스만으로는 구조적으로 놓친다.

- **⚠️ OpenRouter 무료 모델 한도: 하루 50건**. 평상시 일일 수집(요약 10~20건)은 여유가 있으나,
  과거 날짜를 여러 개 백필하면 금방 소진되어 `429`가 난다. 이때 기사 링크는 저장되지 않고
  다음 실행에서 재시도되므로 데이터가 깨지지는 않는다.
- **⚠️ 네이버 뉴스 API는 최신순 30건만 준다.** 2주 이상 지난 날짜는 당일 기사가 창 밖으로 밀려
  백필이 거의 안 된다 (8/17이 1.3%에 머문 이유). 백필은 최대한 이른 시점에 돌려야 한다.
